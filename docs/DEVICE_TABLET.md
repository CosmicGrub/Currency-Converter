...
`device/galaxy-tab`, combined with the Fold5 and alerts/presets branches
into one unified default build by request — see `CHANGELOG.md`). The
`versionName` suffix mentioned below no longer applies (main uses a plain
version number now); everything else still describes exactly what this
code does.
