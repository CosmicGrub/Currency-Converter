...
export default function App() {
  ...
  return (
    ...
    >
      ...
      {/* .eb-container: base (mobile/split-screen) layout lives inline
          further up in this file's history -- now owned by
          src/styles/responsive.css so the stylesheet's media queries can
          flip display:flex -> grid on a wide tablet viewport without
          needing `!important` anywhere. `flex: 1` stays inline since it's
          about sizing within *this* div's own parent (the page-level flex
          column above), not about this div's own children -- orthogonal
          to what the stylesheet controls. */}
      <div className="eb-container" style={{ flex: 1 }}>
        ...
        {/* Basket before Matrix in source order, matching the original
            stacking order -- matters only in the narrow/split-screen
